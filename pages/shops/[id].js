...
export default function ShopDetail() {
  ...
  function shareOrderLink() {
    const link = `${window.location.origin}/order/${data.shop.order_token}`;
    const msg = `Hi ${data.shop.name}! 👋\n\nYou can now place orders directly from your phone:\n\n🛍️ *${link}*\n\nJust tap the link, pick your products, and submit — we'll deliver!\n\nBookmark this link for easy reordering. 📌`;
    const phone = waPhone(data.shop.phone);
    const encoded = encodeURIComponent(msg);
    const url = phone
      ? `https://wa.me/${phone}?text=${encoded}`
    ...
    <>
      ...
      <div style={{ maxWidth: 520, margin: '0 auto', minHeight: '100vh', background: '#eaecf2', paddingBottom: 24 }}>
        ...
        <div style={{ background: 'linear-gradient(160deg, #0d1b2a 0%, #0f3460 100%)', padding: '20px 18px 16px', color: '#fff' }}>
          ...
          <h1 style={{ fontSize: 21, fontWeight: 800, letterSpacing: '-0.3px' }}>{shop.name}</h1>
        ...
        {/* Edit Form */}
        {editing && (
          <form onSubmit={saveEdit} style={{ background: '#fff', margin: '12px', borderRadius: 16, padding: '16px', boxShadow: '0 2px 16px rgba(15,52,96,0.07)' }}>
        ...
        {/* Payment Form */}
        {paying && (
          <form onSubmit={recordPayment} style={{ background: '#fff', margin: '12px', borderRadius: 16, padding: '16px', boxShadow: '0 2px 16px rgba(15,52,96,0.07)' }}>
        ...
        <div style={{ padding: '12px 12px 4px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          <Link href={`/bills/new?shop=${id}`}>
            <div style={{ background: '#0f3460', color: '#fff', borderRadius: 16, padding: '13px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {/* Order Link */}
          {data.shop.order_token && (
            <div style={{ background: '#f3e5ff', borderRadius: 16, padding: '12px 14px', border: '1.5px solid #d9b3ff' }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#8e44ad', marginBottom: 8 }}>🛍️ SHOP ORDER LINK</div>
              <div style={{ fontSize: 12, color: '#666', background: '#fff', borderRadius: 8, padding: '7px 10px', marginBottom: 8, wordBreak: 'break-all', fontFamily: 'monospace' }}>
                /order/{data.shop.order_token}
              </div>
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={shareOrderLink}
                  style={{ flex: 2, padding: '9px', background: '#25D366', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
                ...
                <button onClick={copyOrderLink}
                  style={{ flex: 1, padding: '9px', background: '#8e44ad', color: '#fff', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 600 }}>
                  📋 Copy
                </button>
              </div>
            </div>
          ...
          <button onClick={sendWhatsApp}
            style={{ width: '100%', padding: '13px 16px', background: '#25D366', color: '#fff', border: 'none', borderRadius: 16, fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ...
            {outstandingDeliveries.length > 0 && (
              <span style={{ background: 'rgba(255,255,255,0.25)', borderRadius: 16, padding: '2px 8px', fontSize: 12 }}>
        ...
        <div style={{ padding: '4px 12px 12px' }}>
          {/* Deliveries */}
          {deliveries.length > 0 && (
            <div style={{ background: '#fff', borderRadius: 16, marginBottom: 12, boxShadow: '0 2px 16px rgba(15,52,96,0.07)', overflow: 'hidden' }}>
          ...
          {/* Payments */}
          {payments.length > 0 && (
            <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 16px rgba(15,52,96,0.07)', overflow: 'hidden' }}>
              <div style={{ padding: '10px 12px', borderBottom: '1px solid #f0f0f0', fontWeight: 700, fontSize: 13, color: '#444' }}>
